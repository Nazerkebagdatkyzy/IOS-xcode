...
struct ClassStatisticsView: View {
    ...
    private func computeStats() {
        ...
        do {
            ...
            var totalPresent = 0   // ⭐ МІНЕ, ОСЫ КЕРЕК

            for st in students {

                // ✅ осы оқушы келген КҮНДЕР (уникал)
                let presentDays = Set(
                    all.filter { $0.student == st && $0.isPresent }
                       .compactMap {
                ...
                // ✅ келген күндер саны
                let presentCount = days.filter { presentDays.contains($0) }.count
